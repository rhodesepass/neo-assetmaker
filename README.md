...
**界面优化**
- 标题栏样式现代化
  - 从纯粉色背景改为深色半透明背景 `rgba(40, 40, 40, 0.7)`
  - 添加圆角效果 `border-bottom-right-radius: 16px`
  - 视觉层次更加清晰，现代化风格更明显

- 关闭按钮交互增强
  - 悬停时显示红色警告背景 `rgba(255, 0, 0, 0.3)`
  - 符合关闭按钮的交互习惯，提供更好的视觉反馈
  - 提升用户操作的安全性感知
...
- 主题图片应用增强
  - 支持深色/浅色模式自动适配
  - 根据主题模式设置不同的背景颜色和透明度
  - header_bar和sidebar都使用主题颜色
  - 添加了状态栏样式支持
  - 整体视觉效果更加精致

- 代码结构优化
  - 用户设置加载逻辑更加清晰
  - 主题图片应用逻辑分离到`_apply_theme_change`方法
  - 代码可维护性提升
